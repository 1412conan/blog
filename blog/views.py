...
from blog.models import Article, Tag, Category, Banner, Link
...
def index(request):
    ...
    tui = Article.objects.filter(tui__id=1)[:3]  # 查询推荐位ID为3的文章
    allarticle = Article.objects.all().order_by('-id')[0:10]#把查询出来的分类封装到上下文里
